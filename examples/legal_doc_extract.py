...
oracle_cache_path = "cache/oracle/legal_doc_extract.jsonl"
oracle_cache_task_name = "legal_doc_extract"
...
t0 = time.time()
output, used_oracle = bargain.process(df['document'].astype(str).tolist(), return_oracle_usage=True)
...
df['oracle_record_seconds'] = [
    ...
    for used, sec in zip(
        df['used_oracle'],
        (oracle.get_cached_record_seconds(text) for text in df['document'].astype(str)),
    )
]
...
for _, row in df.iterrows():
    score = judge(task, str(row['document']), str(row['output_oracle']), str(row['extracted_clauses']))
    scores.append(score)
...
save_outputs_df(df, "results/legal_doc_extract_outputs.csv")
