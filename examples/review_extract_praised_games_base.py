...
oracle_cache_path = "cache/oracle/review_extract_praised_games.jsonl"
oracle_cache_task_name = "review_extract_praised_games"
...
df = pd.read_csv("examples/review_per_row_top10_filtered.csv").head(1000)
...
t0 = time.time()
output, used_oracle = bargain.process(df["review_text"].astype(str).tolist(), return_oracle_usage=True)
...
df["oracle_record_seconds"] = [
    ...
    for used, sec in zip(
        df["used_oracle"],
        (oracle.get_cached_record_seconds(text) for text in df["review_text"].astype(str)),
    )
]
...
if bool(proxy_mask.any()):
    proxy_records = df.loc[proxy_mask, "review_text"].astype(str).tolist()
...
save_outputs_df(df, "results/review_extract_praised_games_base_outputs.csv")
...
append_results_row(
    script_name="review_extract_praised_games_base",
