...
t0 = time.time()
output, used_oracle = bargain.process(df['review_text'].to_numpy(), return_oracle_usage=True)
elapsed = time.time() - t0
df['extracted_games'] = output
...
df['output_oracle'] = df['extracted_games']
proxy_mask = ~df['used_oracle']
if proxy_mask.any():
    df.loc[proxy_mask, 'output_oracle'] = oracle.get_pred(
        df.loc[proxy_mask, 'review_text'].to_numpy()
    )

judge = DSPySemanticJudge()
scores = []
for _, row in df.iterrows():
    score = judge(task, row['review_text'], row['output_oracle'], row['extracted_games'])
    scores.append(score)
...
with open(results_csv, 'a', newline='') as f:
    ...
    writer.writerow(['review_extract_praised_games', 'semantic_f1', f'{mean_f1:.3f}', f'{proxy_fraction:.2f}', f'{elapsed:.1f}'])
